...
def init_postgres(con_db):
    con_db.execute("CREATE EXTENSION IF NOT EXISTS vector;")
    con_db.execute("CREATE SCHEMA IF NOT EXISTS corpus;")
    con_db.execute("""
...
def main():
    ...
    db_path = config.get("db_path") or os.getenv("DATABASE_URL")
    log(f"Database: {db_path}")
    log(f"LLM Model: {config.get('llm_model')}")
    log(f"Embedding Model: {config.get('embedding_model')}")
    
    # 1. Connect to Postgres and retrieve existing UIDs
    log("Connecting to Postgres...")
    con_db = psycopg.connect(db_path)
    init_postgres(con_db)
    existing_res = con_db.execute("SELECT uid FROM corpus.newsletters;").fetchall()
    existing_uids = {r[0] for r in existing_res}
    con_db.close()
    log(f"Found {len(existing_uids)} existing records in Postgres.")
            ...
            try:
                ...
                for item in valid_results:
                    ...
                    """, (
                        ...
                        vector_literal(item["english_embedding"]),
                        vector_literal(item["multilingual_embedding"]),
                        item["raw_email"]
                    ))
                ...
                try:
                    ...
                    pass
                log(f"CRITICAL: Error committing chunk {chunk_num} to Postgres: {e}", level="ERROR")
                traceback.print_exc()
